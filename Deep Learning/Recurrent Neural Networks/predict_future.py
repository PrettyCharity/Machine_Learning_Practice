...
class FutureStockPredictor:
    ...
    def fetch_and_prepare_data(self) -> pd.DataFrame:
        ...
        # The future prediction relies heavily on the current sentiment baseline
        current_sentiment = self._fetch_sentiment()

        # Simulate the historical array around the baseline
        df["Sentiment"] = current_sentiment + np.random.normal(0, 0.1, size=len(df))
        df["Sentiment"] = df["Sentiment"].clip(-1, 1)

        # Set the current latest row to the exact current sentiment
        df.loc[df.index[-1], "Sentiment"] = current_sentiment
    ...
    def predict_future(self, model: GRU, period: int) -> Tuple[float, float, str]:
        ...
        with torch.no_grad():
            for _ in range(period):
                ...
                # To feed the prediction back in, we must reconstruct a full feature row.
                # In a robust system, we would predict High/Low/Close/Vol/Sentiment as well.
                # Here, we will copy the last known feature row, update the 'Open' value to our prediction,
                # and append it to our sliding window.
...
def main():
    parser = argparse.ArgumentParser(
        description="Predict future stock direction and price using GRU and Sentiment Analysis."
    )
    parser.add_argument(
        "--ticker",
        type=str,
    ...
    predictor = FutureStockPredictor(args.ticker)
    ...
    print(
        f"Training GRU model with {len(predictor.features)} features (including Sentiment)..."
    )
